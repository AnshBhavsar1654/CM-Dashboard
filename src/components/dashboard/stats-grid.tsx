...
export function StatsGrid({ data }: StatsGridProps) {
  const stats = useMemo(() => {
    ...
    const categorizedData = data.reduce((acc, event) => {
      ...
      if (governmentTypes.includes(eventTypeLower)) acc.governmentEvents++;
      else if (publicTypes.includes(eventTypeLower)) acc.publicEvents++;
      else if (socialTypes.includes(eventTypeLower)) acc.socialEvents++;
      else if (culturalReligiousTypes.includes(eventTypeLower)) acc.culturalReligiousEvents++;
      else if (politicalTypes.includes(eventTypeLower)) acc.politicalEvents++;
      else if (otherTypes.includes(eventTypeLower)) acc.otherEvents++;
      return acc;
    }, {
      governmentEvents: 0,
      publicEvents: 0,
  ...
  const allEventCards = [
    { component: <CulturalReligiousEventsCard value={stats.culturalReligiousEvents} events={data} />, key: "culturalReligious" },
  ...
  return (
    <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-1 w-full h-full min-h-[320px]">
